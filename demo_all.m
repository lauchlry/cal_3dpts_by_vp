...
[vp f u0 v0] = compute_vp(lines, size(img));%liu
...
disp_omap(omap, img, 0.6,kpts,vp,f,u0,v0);%liu 
%[omapmore, OMAP_FACTOR] = compute_omap(linesmore, vp, size(img));
%disp_omap(omapmore, img, 0.6);

%% Generate cuboid hypotheses from omap
cuboidhyp_omap = generate_cuboid_from_omap(omap, vp, OMAP_FACTOR);
...
roomhyp = sample_roomhyp(1000, lines_ex, vp, size(img));
